...
import '../widgets/ObicnaRecenzijaWidget.dart';

class ProductPage extends StatefulWidget {
  ProductPage({Key? key, required this.idProizvod}) : super(key: key);
...
Future<bool> changedata(status, int idProizvod) async {
  if (status == false) {
    //dodaj u listu zelja
    dodaj_u_listu_zelja(idProizvod);
  } else {
    obrisi_iz_lz(idProizvod);
  }
  return Future.value(!status);
}

Future<void> dodaj_u_listu_zelja(int idProizvod) async {
  var url = Uri.parse("http://localhost:8000/dodaj_u_lz");
  ...
  var response = await http.post(
    ...
    body: jsonEncode(<String, int>{
      ...
      'idKorisnik': Manager().user!.id, //OVDE DODATI MANAGER.USER.iD
    }),
  );
  var bod = jsonDecode(response.body);
  var b = bod['success'];
...
Future<void> obrisi_iz_lz(int id) async {
